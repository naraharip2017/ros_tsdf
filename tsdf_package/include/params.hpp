...
struct Params{
    ...
    Params(float voxel_size_input, float truncation_distance_input, float max_weight_input, float publish_distance_squared_input, float garbage_collect_distance_squared_input){
        ...
        float * garbage_collect_distance_squared_param_h = new float(garbage_collect_distance_squared_input);
        cudaMalloc(&garbage_collect_distance_squared_param_d, sizeof(*garbage_collect_distance_squared_param_h));
        cudaMemcpy(garbage_collect_distance_squared_param_d, garbage_collect_distance_squared_param_h, sizeof(*garbage_collect_distance_squared_param_h), cudaMemcpyHostToDevice);
        delete garbage_collect_distance_squared_param_h;
    }
    ~Params(){
        ...
        cudaFree(garbage_collect_distance_squared_param_d);
    }
};
